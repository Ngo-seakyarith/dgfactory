...
export function SystemProposalWorkspace({ id }: { id?: string }) {
  ...
  return <div className="space-y-5">
    <header className="flex flex-wrap items-end justify-between gap-4 border-b border-border pb-5"><div className="page-heading"><Button asChild variant="ghost" className="mb-1 -ml-3"><Link href="/system-proposals"><ArrowLeft className="h-4 w-4" />System proposals</Link></Button><div className="page-eyebrow">System consulting</div><h1 className="page-title">{id ? proposal.brief.projectTitle || "Intelligent system proposal" : "New system proposal"}</h1><p className="page-description">Turn client data and reviewed evidence into a practical system recommendation.</p></div><div className="flex items-center gap-2">{id ? <Badge variant="teal">{proposal.status}</Badge> : null}<Button variant="outline" onClick={() => void save()} disabled={Boolean(busy)}><Save className="h-4 w-4" />Save</Button>{id ? <Button variant="destructive" size="icon" title="Delete project" onClick={() => void deleteProject()} disabled={Boolean(busy)}><Trash2 className="h-4 w-4" /></Button> : null}</div></header>
    <div className="grid gap-px overflow-hidden rounded-md border border-border bg-border sm:grid-cols-3">{stages.map((item) => { const Icon = item.icon; const active = stage === item.id; return <Button key={item.id} variant="ghost" className={`h-12 justify-start rounded-none bg-card ${active ? "border-b-2 border-b-[#f4772e] text-foreground" : "text-muted-foreground"}`} onClick={() => setStage(item.id)}><Icon className={`h-4 w-4 ${active ? "text-[#a94b18]" : ""}`} />{item.label}</Button>; })}</div>
    ...
    {notice.text ? <p className={`rounded-md border p-3 text-sm ${notice.error ? "border-destructive/25 bg-destructive/10 font-medium text-destructive" : "border-teal-300/25 bg-teal-300/10 text-teal-50"}`}>{notice.text}</p> : null}
