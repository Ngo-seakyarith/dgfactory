...
- Capture a structured proposal brief aligned to DG Academy cover, course design, schedule, trainers, fee, logistics, and acceptance sections
- Proposal DOCX export mirrors the approved DG Academy format with a full-page cover, numbered course sections, complete selected trainer profiles, no-table acceptance layout, and deterministic Commercial Setup fee. Long trainer profiles continue onto additional pages.
- Every DOCX export embeds the DG Academy logo from `public/app-logo.png`.
- Package creation requires a primary trainer and allows one optional second trainer from the 17 approved DG Academy profiles in `src/features/training-packages/data/trainers.json`. Duplicate selection is blocked and each selection fills a read-only catalog profile.
- Proposal DOCX exports download each selected trainer's approved PNG or JPEG portrait from ImageKit and include the complete biography, experience, and qualifications.
...
The primary trainer and optional second trainer are stored as snapshots inside the existing `proposal_brief` JSONB value, including trainer IDs, ImageKit URLs, biographies, experience, and qualifications. This requires no additional Supabase table or migration. Existing packages without a trainer ID remain readable, but users must select an approved primary trainer before regeneration or proposal/syllabus DOCX export.
...
- Before Training: run a Pre-Training Assessment (AI-drafted baseline questions shared through the same private-link survey flow so the trainer can tailor the session), generate the training materials, confirm the compact logistics (date, venue, trainer, participants, notes), and work the preparation checklist.
- Training Materials: AI drafts a layout-aware slide plan (branded PPTX export), a 6-10 page participant workbook (DOCX), a trainer-facing facilitator guide (DOCX), and an optional AI prompt library for participants (DOCX) from the original saved package brief and confirmed delivery details; each material is stored on the delivery, previewable in the browser, and regenerable. Slide generation uses a version-2 structured contract shared with the PptxGenJS renderer. Alongside section, statement, bullets, numbered, two-column, practice, and closing layouts, the model can choose editable visual layouts for icon concepts, processes, cycles, comparisons, matrices, timelines, funnels, and evidence-based charts. Visual content is supplied as structured labels, descriptions, axes, and numeric values; the exporter controls geometry, icons, connectors, spacing, and DG Academy styling. Charts are allowed only when the package provides real numeric evidence. Version-1 saved slide plans and existing free-form Markdown materials remain exportable through normalization and the legacy fallback. Workbook, facilitator-guide, and prompt-library generation also use versioned structured contracts shared with their dedicated Word renderers. The workbook exports participant activities, response space, reusable templates, and an action plan; the facilitator guide exports a timed agenda, detailed run sheets, debriefs, materials, participant questions, and contingencies; the prompt library exports grouped copy-ready prompts with adaptation and review checks. The server stores readable Markdown with hidden structured plans so the browser preview and export stay synchronized.
...
- DOCX exports use `docx`, PPTX exports use the pinned `pptxgenjs` renderer, and Markdown exports remain plain UTF-8 text.
