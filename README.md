...
- Training Materials: AI drafts a layout-aware slide plan (branded PPTX export), a 6-10 page participant workbook (DOCX), a trainer-facing facilitator guide (DOCX), and an optional AI prompt library for participants (DOCX) from the original saved package brief and confirmed delivery details; each material is stored on the delivery, previewable in the browser, and regenerable. Slide generation uses a structured contract shared with the PptxGenJS renderer: the model selects `section`, `statement`, `bullets`, `numbered`, `two-column`, `practice`, or `closing` for each slide and must keep its content inside that layout's limits. Teaching layouts support an explanatory introduction plus richer points or steps, and section slides include an overview and learning focus instead of a title alone. Complex topics should expand across multiple complete slides rather than become sparse headings or overflowing text. Workbook, facilitator-guide, and prompt-library generation also use versioned structured contracts shared with their dedicated Word renderers. The workbook exports participant activities, response space, reusable templates, and an action plan; the facilitator guide exports a timed agenda, detailed run sheets, debriefs, materials, participant questions, and contingencies; the prompt library exports grouped copy-ready prompts with adaptation and review checks. The server stores readable Markdown with hidden structured plans so the browser preview and export stay synchronized. Existing free-form Markdown materials remain exportable through the legacy DOCX fallback.
